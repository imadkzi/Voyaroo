...
export async function PATCH(
  ...
  const body = (await request.json().catch(() => null)) as
    | { dayNumber?: number; title?: string; notes?: string }
    | null;
  ...
  const rows = await query<{
    ...
    `update itinerary_items
     ...
     returning id::text as id, day_number, title, notes`,
    [itemId, tripId, dayNumber ?? null, title ?? null, notes ?? null],
  );
  ...
  return NextResponse.json({
    ok: true,
    item: r
      ? { id: r.id, dayNumber: r.day_number, title: r.title, notes: r.notes }
      : null,
  });
}
...
export async function DELETE(
  ...
  await query(`delete from itinerary_items where id = $1 and trip_id = $2`, [
    itemId,
    tripId,
  ]);
